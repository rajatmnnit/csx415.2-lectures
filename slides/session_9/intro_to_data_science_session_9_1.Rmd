...
- Final exam (homework) is cumulative (including today's material) and will be in two parts:
  ...
  + Take home portion assigned today in place of a homework assignment (Due by Friday August 10 at noon)
  
  + In class portion will be completed in class on the last day (Aug 7), and due in class by 9:30PM. 
...
Suppose $\huge X_{1}$ is a categorical variable with $\huge k$ categories. We create $\huge k-1$ dummy variables.
...
$$\Huge \hat{Price} = \hat{\beta_{0}} + \hat{\beta_{1}} \times cut$$

becomes this:

$$\Huge \hat{Price} = \hat{\beta_{0}} + \hat{\beta_{1}} \times Good + \hat{\beta_{2}} \times VeryGood + \hat{\beta_{3}} \times Premium + \hat{\beta_{4}} \times Ideal$$
---
### Dummy variables
...
$$\Huge \hat{Price} = \hat{\beta_{0}} + \hat{\beta_{1}}$$

and we interpret $\huge \hat{\beta_{1}}$ as the mean difference in `Price` between `Fair` and `Good` diamonds. 
---
class: inverse, center, middle

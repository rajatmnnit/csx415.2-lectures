...
We want to know $\huge p_{A}$ and $\huge p_{B}$: the true conversion rates for the entire population for Version A and Version B


We estimate this using $\huge \hat{p}_{A}$ and $\huge \hat{p}_{B}$: the conversion rates from a sample of visitors to Version A and Version B
---
### The Sample Statistic
...
$\huge p =$ population proportion

$\huge \hat{p} =$ sample proportion

$$\huge \hat{p} \sim N\left(p, \sqrt{\frac{p(1-p)}{n}}\right)$$
  
The standard error of $\huge \hat{p}$ is

$$\huge \sqrt{\frac{p(1-p)}{n}}$$
  
as long as $\huge np \geq 10$ and $\huge n(1-p) \geq 10$
...
Suppose we have a process (such as flipping a coin) that only has two possible outcomes (failure and success), and that the outcome we are interested in happens with a constant probability of $\huge p$. And suppose we observe $\huge n$ number of trials of that process.

Let $\huge X$ be the number of successes we observe out of those $\huge n$ trials.

Example:

We flip a fair coin 100 times. The *success* in this case is landing on heads. The probability of landing on heads is 0.5. The number of heads, out of 100 coin flips, follows a binomial distribution with $\huge n = 100, p = 0.5$.
...
$\huge X_{A}:$ the number of version A visitors who convert, and 
$\huge X_{B}:$ the number of version B visitors who convert

follow Binomial distributions, with $\huge n = 1000$, and $\huge p =$ probability of conversion.

---
### The binomial distribution

$\huge X_{A}$ follows a binomial distribution that looks like this:
...
Recall the definition: the p-value is the probability of observing a statistic as extreme, or more extreme, than what we got, if the null is true. If p-value < $\huge \alpha$, we reject the null hypothesis.
...
- alternative: default is $\huge \neq$
- conf.level: $\huge 1 - \alpha$, default is 0.95
...
The p-value is small (less than some value $\huge \alpha$ that we choose, typically 0.05), so we decide that our null hypothesis is wrong, and that the true conversion rate **IS NOT** 0.1.
---
### Pop Quiz
...
$$\huge H_{alt}: p_{A} \neq p_{B}$$
$$\huge H_{alt}: p_{A} > p_{B}$$
$$\huge H_{alt}: p_{A} < p_{B}$$
---
### The alternative hypothesis

The alternative hypothesis you choose is problem-specific, but to make it easy, we will always be testing versus the alternative hypothesis that $\huge p_{A} \neq p_{B}$.
---
...
$$\huge H_{0}: p_{A} - p_{B} = 0$$

Alternatives:

$$\huge H_{alt}: p_{A} - p_{B} \neq 0$$
$$\huge H_{alt}: p_{A} - p_{B} > 0$$
$$\huge H_{alt}: p_{A} - p_{B} < 0$$
---
### The hypothesis test - redefined
...
$$\huge \hat{p}_{A} - \hat{p}_{B} \sim N\left(p_{A}-p_{B}, \sqrt{\frac{p_{A}(1-p_{A})}{n_{A}} + \frac{p_{B}(1-p_{B})}{n_{B}}}\right)$$
---
### Hypothesis testing fundamental idea
...
Each type of error has a certain *probability* of occurring. The probability of a type I error is $\huge \alpha$. Think about that for a second...
